...
我们所创建的每一个函数，解析器都会**向函数中添加一个属性 `prototype`**。这个属性对应着一个对象，这个对象就是我们所谓的原型对象。

如果函数作为普通函数调用`prototype`没有任何作用，当函数以构造函数的形式调用时，它**所创建的实例对象中都会有一个隐含的属性，指向该构造函数的原型，我们可以通过`__proto__`来访问该属性**。
