...
不难想象，如果依次读取两个以上的文件，就会出现**多重嵌套**。代码不是纵向发展，而是横向发展，很快就会乱成一团，无法管理。因为多个异步操作形成了强耦合，**只要有一个操作需要修改，它的上层回调函数和下层回调函数，可能都要跟着修改。这种情况就称为"回调函数地狱"（callback hell）**。
...
上面代码的函数`asyncJob`是一个协程，它的奥妙就在其中的yield命令。它表示执行到此处，执行权将交给其他协程。也就是说，**`yield`命令是异步两个阶段的分界线**。
...
换言之，`next`方法的作用是**分阶段执行Generator函数**。每次调用next方法，会返回一个对象，表示当前阶段的信息（value属性和done属性）。`value`属性是yield语句后面表达式的值，表示当前阶段的值；`done`属性是一个布尔值，表示 Generator 函数是否执行完毕，即是否还有下一个阶段。
...
上面代码中，首先执行 Generator 函数，获取遍历器对象，然后使用next方法（第二行），执行异步任务的第一阶段。由于`Fetch模块返回的是一个 Promise 对象`，因此要用then方法调用下一个next方法。
